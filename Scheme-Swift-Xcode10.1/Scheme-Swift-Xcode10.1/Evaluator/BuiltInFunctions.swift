...
func plus(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    ...
    if args.count == 0 {
        return .Error(message: "(+) - Enter at least one value", line: #line, function: #function, file: #file)
    }
    var sum = 0
    for integer in args {
        guard let value = integer.intValue() else {
            return .Error(message: "(+) - Only integers are valid", line: #line, function: #function, file: #file)
        }
        sum = sum + value
    }
...
func remainder(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object  {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 2 {
        return .Error(message: "(%) - Needs exactly two arguments", line: #line, function: #function, file: #file)
...
func truncate(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(truncate) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
    ...
    guard let value = args.first?.intValue() else {
        return .Error(message: "(truncate) - Needs a number value", line: #line, function: #function, file: #file)
    }
...
func eq(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 2 {
        return .Error(message: "(eq?) - Needs exactly two arguments", line: #line, function: #function, file: #file)
...
func cons(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 2 {
        return .Error(message: "(cons) - Needs exactly two arguments", line: #line, function: #function, file: #file)
    }
...
func car(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(car) - Needs exactly two arguments", line: #line, function: #function, file: #file)
    }
    guard let cons = args[0].consObject() else {
        return .Error(message: "(car) - Argument is not a cons", line: #line, function: #function, file: #file)
    }
...
func cdr(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(cdr) - Needs exactly two arguments", line: #line, function: #function, file: #file)
    }
    guard let cons = args[0].consObject() else {
        return .Error(message: "(cdr) - Argument is not a cons", line: #line, function: #function, file: #file)
    }
...
func display(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    ...
    if args.count == 0 {
        return .Error(message: "(display) - Needs at least one argument", line: #line, function: #function, file: #file)
    }
...
func print(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    ...
    if args.count == 0 {
        return .Error(message: "(print) - Needs at least one argument", line: #line, function: #function, file: #file)
    }
...
func load(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(load) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
    ...
    guard let path = args.first?.stringObject()?.characters else {
        return .Error(message: "(load) - Needs a string path", line: #line, function: #function, file: #file)
    }
    ...
    do {
        ...
        return .Error(message: "(load) - the given path is not valid", line: #line, function: #function, file: #file)
    }
}
...
func stringLength(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(string-length) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
    guard let string = args.first?.stringObject() else {
        return .Error(message: "(string-length) - Needs a string as argument", line: #line, function: #function, file: #file)
    }
...
func stringRef(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 2 {
        return .Error(message: "(string-ref) - Needs exactly two argument", line: #line, function: #function, file: #file)
    }
    guard let string = args.first?.stringObject() else {
        return .Error(message: "(string-ref) - Needs a string as first argument", line: #line, function: #function, file: #file)
    }
    guard let index = args.last?.intValue()  else {
        return .Error(message: "(string-ref) - Needs a string as second argument", line: #line, function: #function, file: #file)
    }
    
    if index >= string.length {
        return .Error(message: "(string-ref) - Index out of bounds", line: #line, function: #function, file: #file)
    }
...
func stringAppend(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count < 1 {
        return .Error(message: "(string-append) - Needs min one argument", line: #line, function: #function, file: #file)
    }
    ...
    for string in args {
        guard let value = string.stringObject()?.characters else {
            return .Error(message: "(string-append) - Only string as arguments are valid", line: #line, function: #function, file: #file)
        }
        retVal.append(value)
    }
...
func stringEqual(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 2 {
        return .Error(message: "(string=?) - Needs exactly two argument", line: #line, function: #function, file: #file)
    }
    guard let firstString = args.first?.stringObject()?.characters else {
        return .Error(message: "(string=?) - Needs a string as first argument", line: #line, function: #function, file: #file)
    }
    guard let secondString = args.last?.stringObject()?.characters  else {
        return .Error(message: "(string=?) - Needs a string as second argument", line: #line, function: #function, file: #file)
    }
...
func stringQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(string?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
...
func boolQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(bool?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
...
func numberQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(number?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
...
func consQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(cons?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
...
func builtinFunctionQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(builtin-function?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
...
func userFunctionQuestion(firstArgIndex: Int, in evalStack: Stack<Object>) -> Object {
    let args = evalStack.getObjects(from: firstArgIndex)
    if args.count != 1 {
        return .Error(message: "(user-function?) - Needs exactly one argument", line: #line, function: #function, file: #file)
    }
