...
private extension Evaluator {
    ...
    private func eval(list: SchemeCons, environment: SchemeEnvironment) -> Object {
        ...
        guard let args = list.cdr.consObject() else {
            return .Error(message: "The list has no arguments", line: #line, function: #function, file: #file)
        }
        ...
        case .UserDefinedFunction(let userDefined):
            ...
            for value in argListValue {
                guard let name = argListName.car.symbolObject() else {
                    return .Error(message: "There went somethin wrong", line: #line, function: #function, file: #file)
                }
    ...
    private func evalSyntax(define: SchemeSyntax, in environment: SchemeEnvironment, with args: SchemeCons) -> Object {
        ...
        if let varName = varOrFunc.symbolObject() {
            guard let restArg = args.cdr.consObject() else {
                return .Error(message: "(define): 2 args expected", line: #line, function: #function, file: #file)
            }
            
            if restArg.cdr != .Null {
                return .Error(message: "(define): only 2 args expected", line: #line, function: #function, file: #file)
            }
            ...
            guard let cons = varOrFunc.consObject() else {
                return .Error(message: "Bad arg. Es sollte ein Cons Objekt sein", line: #line, function: #function, file: #file)
            }
            
            guard let funcName = cons.car.symbolObject() else {
                return .Error(message: "Bad function name. Es sollte ein Symbol Objekt sein", line: #line, function: #function, file: #file)
            }
            
            guard let argList = cons.cdr.consObject() else {
                return .Error(message: "Bad arg name list", line: #line, function: #function, file: #file)
            }
            ...
            guard let bodyList = args.cdr.consObject() else {
                return .Error(message: "Body List is no cons", line: #line, function: #function, file: #file)
            }
    ...
    private func evalSyntax(set: SchemeSyntax, in environment: SchemeEnvironment, with args: SchemeCons) -> Object {
        guard let varName = args.car.symbolObject(),
              ...
              let unevaluatedVarValue = args.cdr.consObject()?.car else {
                return .Error(message: "(set!): 2 args expected", line: #line, function: #function, file: #file)
        }
        
        if restArgs.cdr != .Null {
            return .Error(message: "(set!): only 2 args expected", line: #line, function: #function, file: #file)
        }
        ...
        if environment.update(key: varName, value: varValue) == false {
            return .Error(message: "(set!) - There is no binding for \(varName.characters)", line: #line, function: #function, file: #file)
        }
    ...
    private func evalSyntax(if: SchemeSyntax, in environment: SchemeEnvironment, with args: SchemeCons) -> Object {
        ...
        guard let trueExpr = args.cdr.consObject()?.car,
            ...
            return .Error(message: "(if): 3 args expected", line: #line, function: #function, file: #file)
        }
        ...
        default:
            return .Error(message: "non binary condition", line: #line, function: #function, file: #file)
        }
    }
    ...
    private func evalSyntax(lambda: SchemeSyntax, in environment: SchemeEnvironment, with args: SchemeCons) -> Object {
        guard let argList = args.car.consObject() else {
            return .Error(message: "Args erwartet", line: #line, function: #function, file: #file)
        }
        guard let bodyList = args.cdr.consObject() else {
            return .Error(message: "Mindestend 2 args erwartet", line: #line, function: #function, file: #file)
        }
    ...
    private func evalSyntax(quote: SchemeSyntax, in environment: SchemeEnvironment, with args: SchemeCons) -> Object {
        if args.car == .Null {
            return .Error(message: "(Quote): 1 arg erwartet", line: #line, function: #function, file: #file)
        }
        
        if args.cdr != .Null {
            return .Error(message: "(Quote): genau 1 arg erwartet", line: #line, function: #function, file: #file)
        }
        return args.car
    }
}
